...
export default function DestinationDetail() {
    ...
    async function fetchDestination() {
      ...
      const localMatch = DESTINATIONS_DATA.find(
        (d) =>
          ...
          d.slug?.toLowerCase() === target ||
          d.name.toLowerCase().includes(
            target.replace(/-/g, ' ')
          )
      );

      let finalDestination = localMatch || null;

      try {
        // If URL contains a numeric database ID
        if (/^\d+$/.test(target)) {
        ...
        } else if (localMatch?.name) {
          // Match database destination by name
          const { data, error } = await supabase
            .from('destinations')
            .select('*')
  ...
  const loadVerifiedGuides = async () => {
    ...
    if (!user) {
      setSubmitError(
        'Please login as a traveler before booking a guide.'
      );
    ...
    if (error) {
      ...
      setSubmitError(
        'Unable to load verified guides. Please try again.'
      );
      setGuides([]);
    } else {
      setGuides(data || []);
  ...
  const openGuideModal = async () => {
    ...
    if (!user) {
      setSubmitError(
        'Please login first to book a tour guide.'
      );
      return;
    }
  ...
  const handleGuideBookingSubmit = async (e) => {
    ...
    if (!user) {
      setSubmitError(
        'Please login before submitting a booking.'
      );
      return;
    }
    ...
    if (!selectedGuideId) {
      setSubmitError(
        'Please select a verified tour guide.'
      );
      return;
    }
  ...
  if (!destination) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-ceylon-bg p-6 text-center">
        ...
        <Link
          to="/"
          className="text-ceylon-accent font-bold underline"
        >
          Back to Home
        </Link>
      </div>
    ...
    <div className="min-h-screen bg-ceylon-bg pb-20">
      ...
      <div className="relative h-[540px] md:h-[640px] w-full bg-slate-950 overflow-hidden shadow-2xl">
        ...
        <div className="absolute bottom-8 left-6 right-6 max-w-7xl mx-auto text-white z-20">
          ...
          <h1 className="text-4xl md:text-6xl font-editorial font-extrabold mb-3 drop-shadow-md">
            {destination.name}
          </h1>

          <div className="flex flex-wrap items-center gap-4 text-sm font-semibold text-slate-200">

            <span className="flex items-center gap-1.5 bg-slate-900/70 px-3 py-1 rounded-lg border border-slate-800">
            ...
            <span className="flex items-center gap-1.5 bg-slate-900/70 px-3 py-1 rounded-lg border border-slate-800">
              ...
              Best Time: {destination.best_time_to_visit || 'Nov to April'}
            </span>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-10 grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            ...
            <h2 className="text-2xl font-bold text-ceylon-primary mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-100">
              ...
              <div className="flex items-start gap-3 p-4 bg-orange-50 rounded-2xl">
                <ShieldCheck
                  className="text-ceylon-accent mt-1"
                  size={24}
                />
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600">
                    Official tourism verified destination
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-4 bg-emerald-50 rounded-2xl">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600">
                    Local certified guides available
                  </p>
                </div>
              </div>
          ...
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            ...
            <h2 className="text-2xl font-bold text-ceylon-primary mb-4 flex items-center gap-2">
              <MapPin className="text-ceylon-accent" />
              Location & Map
            </h2>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-lg border border-orange-100 sticky top-6">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Need a licensed tour guide or private vehicle for{' '}
              {destination.name}?
            </p>
              ...
              {bookingSubmitted ? (
                <div className="text-center py-8 space-y-4">
                  ...
                  <p className="text-sm text-slate-300">
                    Your booking request has been sent to the selected
                    verified guide.
                  </p>
                ...
                <form
                  onSubmit={handleGuideBookingSubmit}
                  className="space-y-4"
                >
                  ...
                  <p className="text-xs text-emerald-400 font-bold mb-4">
                    Connect with official SLTDA verified tour guides.
                  </p>

                  {/* GUIDE SELECT */}
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">
                      Select Verified Tour Guide
                    </label>
                    ...
                    {guidesLoading ? (
                      ...
                      <div className="bg-red-950/40 border border-red-500/30 rounded-xl p-3 text-sm text-red-300">
                        No verified guides are currently available.
                      </div>
                    ) : (
                      <select
                        required
                      ...
                      >
                        ...
                        {guides.map((guide) => (
                          <option
                            ...
                            value={guide.id}
                          >
                            {guide.full_name || 'Licensed Guide'}
                            {guide.district
                              ? ` — ${guide.district}`
                              : ''}
                            {guide.rating
                  ...
                  {/* ERROR */}
                  {submitError && (
                    <div className="bg-red-950/50 border border-red-500/30 text-red-300 text-xs rounded-xl p-3">
                      {submitError}
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:text-slate-400 text-white font-extrabold py-3.5 rounded-xl transition-all shadow-lg mt-4 cursor-pointer disabled:cursor-not-allowed"
                  >
        ...
        createPortal(
          ...
          >
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl relative text-center text-white max-h-[85vh] overflow-y-auto custom-scrollbar my-auto">
              ...
              <h3 className="text-2xl font-editorial font-extrabold text-white mb-2">
                Train Ticket Inquiry
              </h3>

              <p className="text-sm text-slate-300 mb-6">
                Scenic train routes connecting{' '}
                <span className="font-bold text-emerald-400">
                  {destination.name}
                </span>{' '}
                (such as Kandy to Ella Express & Ella Odyssey)
                must be reserved 30 days in advance.
              </p>
              ...
              <div className="p-4 bg-emerald-950/60 border border-emerald-500/30 rounded-2xl text-left text-xs text-emerald-300 mb-6 space-y-1">
                <p className="font-bold text-emerald-400">
                  🚆 Official Ticket Reservation Info:
                </p>

                <p>• 1st Class Observation Car</p>
